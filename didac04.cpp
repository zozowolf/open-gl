 ...
 * didac04.cpp
 *
 * Affichage d'un bras robot                             
 ********************************************************
 *  Objectif : Gestion des piles de matrice
 ********************************************************/
...
float xcam=0 , ycam=0 , zcam=10 , Rotation=0 , tx=0 , ty=0 , r1=0 ,r2=0 , r3=0 , r4=0;
bool Zoom=false, Tourne=false , Deplace=false;


GLfloat L0pos[]={ 10.0,10.0,20.0};
GLfloat L0dif[]={ 0.1,0.1,0.1};
GLfloat L1pos[]={-10.0,10.0,-20.0};
GLfloat L1dif[]={ 0.1,0.1,0.1};
...
int main(int argc,char **argv)
{
	...
	glutCreateWindow("Youpi, le petit robot");
...
/***********************************************************************
 Dessin d'un bras
 ***********************************************************************/
void DessineAxe(float longueur, float largeur , float hauteur)
{
	glScalef(longueur , largeur , hauteur);
	glutSolidCube(1.0);
}
...
void display()
{
	...
	glTranslatef(tx,ty,0);		// Translation de toute la figure
	glRotatef(Rotation,0,1,0);	// Rotation de toute la figure
	glPushMatrix();				// Sauvegarde de cette config

	DessineAxe(3,0.5,3);
	glPopMatrix();				// Récupération config	
		
	glRotatef(r1,0,0,1);		// 2 ==> rotation
	glTranslatef(1 ,0 ,0);		// 1 ==> décalage de 1 pour définir axe rotation 
		
	glPushMatrix();				// Sauvegarde de cette config
	DessineAxe(2,0.5,0.5);				// Dessin du bras 
	
	glPopMatrix();				// Récupération matrice 
	glTranslatef(1 ,0 ,0);
	glRotatef(r2,0,0,1);
	glTranslatef(1 ,0 ,0);

	glPushMatrix();
	DessineAxe(2,0.5,0.5);
...
void clavier(unsigned char touche,int x,int y)
{
	...
	case ' ': if (r4==30) r4=0; else r4=30;break;
